...
#
    ...
    scripts_dir="/ap-scripts/"
    log_dir="/var/log/ap-backups/"
    backupscript="/ap-scripts/ap-backups.sh"
	...
	read -p "Are You Running A Webservice? Please Select [y/N]" yn
		...
		* )
	    ...
	    echo "" >> $backupconfig
	    echo "# Extra Non-Vhost Directory's To Include In The Backups;" >> $backupconfig
	    echo ""
            ...
            while true; do
			    ...
			    while true ; do
				    ...
				    while true; do
						    ...
						    if [ $extlastchar != "/" ]; then # Checking If The Path Ends In A Slash Otherwise It Could Break The Backup Script
							echo "backup_directory='$extradir/'" >> $backupconfig;
						    else
							echo "backup_directory='$extradir'" >> $backupconfig;
						    fi
			    ...
			    echo "backup_directory=''" >> $backupconfig;
			    break;;
                    * ) 
			    echo "Please Select [y/N]";;
                esac
	    ...
	    echo "" >> $backupconfig
            ...
            # Giving The User A Choice To Enter Any MySQL Backups They Wish To Exclude From The Backups

	    echo "# Databases That Are Not To Be Included In The Backups;" >> $backupconfig
	...
	echo "By Default All Databases Are Included In The Backup, If You Have Specific Databases You Do Not Wish To Backup You Can Do This Below"
            ...
            while true; do
                    ...
                    [Yy]* )
                            ...
                            while true ; do
                                ...
                                if [ "$excludedb" != "finish" -a "$excludedb" != "Finish" ]; then
                                    while true; do
                                        read -p "You Have Entered The Directory $excludedb. Is This Correct? Please Select [y/N]" yn
                                            case $yn in
                                            [Yy]* ) echo "exclude_database='$excludedb'" >> $backupconfig;
			    ...
			    echo "exclude_database=''" >> $backupconfig;
                            break;;
                    * )
                            echo "Please Select [y/N]";;
                esac
        ...
        # Create function for proceeding with old config file, at this point we will check the config file incase there is some modification that we do not want. We won't bother checking for extra backup locations being valid as this will be performed on the fly in the backup script.
        oldconfig()     {
		...
		testbackupdays=$(grep "total_backup_days" $backupconfig | awk -F\' '{print $2}')
			...
			newconfig;
		        ...
		        echo "Time To Trigger Backup Compression Is Valid"
                        ...
                        echo "Verification Check Of The Original Configuration File Failed, Moving The Old Con
fig File To $backupconfigold & Starting Process To Create New Confg File"
                        mv $backupconfig $backupconfigold;
                        newconfig;
                    fi
                        }
	...
	if crontab -l | grep "/bin/bash /ap-scripts/ap-backups.sh"; then 
	    ...
	    crontab -l | { cat; echo "0 22 * * *  /bin/bash /ap-scripts/ap-backups.sh"; } | crontab -
	fi
