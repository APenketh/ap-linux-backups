...
# Add in some checking for the size if the previous backup does not exist
...
#
    ...
    # Redirect stdout to file log.out then redirect stderr to stdout. Note that the order is important whe                                                           n you want them going to the same file. stdout must be redirected before stderr is redirected to stdout
    ...
    echo "$(datetime) Starting The File Level Backup Process"
        ...
        # Backing up the extra locations as per the config file
            if [ "$extradirbackup" = "" ]; then
           echo "$(datetime) There Are No Extra Directory's Defined. Proceeding"
       else
           for extradir in $extradirbackup; do
               if [ -d $extradir ]; then
                    if [ ! -f $backup_dir/$vhostname-$timestamp.tar.gz ]; then
                        vhostname=`echo "$vhost" | awk -F/ '{print $NF}'`
                        echo "$(datetime) Starting File Level Backup For $vhostname"
